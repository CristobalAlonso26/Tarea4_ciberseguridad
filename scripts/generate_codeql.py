...
class CodeQLAnalyzer:
    ...
    def discover_repositories(self) -> list[dict]:
        """Devuelve la lista de repositorios desde repos_activos.json."""
        repos_json_path = self.project_root / "data" / "results" / "repos_activos.json"
        if not repos_json_path.exists():
            LOGGER.warning(f"No se encontró {repos_json_path}")
            return []
    ...
    def run_codeql(self, repo_path: str, language: str | None = None) -> str:
        ...
        lenguaje = language or self._detectar_lenguaje_simple(ruta_repo)
        if not lenguaje:
            LOGGER.warning(
    ...
    def run(self):
        """Orquesta el descubrimiento y clonado, y análisis con CodeQL."""
        ...
        for indice, repo_data in enumerate(repositorios, start=1):
            repo_name = repo_data["name"]
            clone_url = repo_data["clone_url"]
            repo_language = repo_data.get("language", "")
            
            # Directorio donde se clonará
            repo_path_rel = str(Path("data/repos") / repo_name)
            ...
            if self.dry_run:
                LOGGER.info("[%s/%s] Dry-run: se analizaría %s", indice, len(repositorios), repo_name)
                continue
                
            codeql_lang = "javascript" if repo_language in ["TypeScript", "JavaScript"] else "python" if repo_language == "Python" else None
            ...
            try:
                ...
                self.save_analysis(repo_name, analysis)
                repositorios_analizados += 1
                archivos_generados += 1
            except Exception as error:
                errores += 1
                self._eliminar_archivos_parciales(repo_name)
                LOGGER.error(
